...
h_pool4 = max_pool_2x2_stride(h_conv4)

#Size: 56*56
W_conv5 = weight_variable([3, 3, 128, 256], 12312323)
b_conv5 = bias_variable([256])
h_conv5 = tf.nn.relu(conv2d(h_pool4, W_conv5) + b_conv5)

W_conv6 = weight_variable([3, 3, 256, 256], 12312323)
...
train_step = tf.train.MomentumOptimizer(lr, m).minimize(cross_entropy)
...
for epoch in range(0, 100000000000000000000000):
	for i in range(num_data):
		dataset_name = ngii_dir[i][0]
		print('Current Dataset: %s (num_data %d)' % (dataset_name, i))

		for j in range(0, steps):
			x_batch, _, y_batch = data.make_batch(dataset_name, 64)
